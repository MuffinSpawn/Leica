...
class CommandPacket(object):
    def __init__(self, packet_size, struct_spec, body):
        self.packetInfo = PacketHeaderT(packet_size=packet_size, packet_type=ES_DT_Command)
        self.data = struct.Struct(struct_spec).pack(self.packetInfo + body)

class PacketHeaderT(object):
    def __init__(self, header_data=None, packet_size=None, packet_type=None):
        if header_data == None:
            self.lPacketSize = packet_size
...
class BasicCommandCT(CommandPacket):
    def __init__(self, packet_size, command, body_spec, body):
        self.packetHeader = PacketHeaderT(packet_size=packet_size, packet_type=ES_DT_Command)
        struct_spec = ''.join(('L I I', body_spec))
        self.data = struct.Struct(struct_spec).pack(self.packeHeader + (command,) + body)


class SetUnitsCT(CommandPacket):
    def __init__(self, command):
        super().__init__(40, ES_C_SetUnits, 'I I I I I', ...)
...
class LTPacketStream(threading.Thread):
    ...
    def run(self):
        ...
        while self.__running:
            header_data = self.__sock.recv(self.PACKET_HEADER_SIZE)
            packet_header = PacketHeaderT(header_data)

            body_data = self.__sock.recv(packet_header.lPacketSize-self.PACKET_HEADER_SIZE)
    ...
    def write(self, packet):
        self.__sock.sendall(packet.data)
